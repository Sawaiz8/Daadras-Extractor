...
def sess_section_page():
    ...
    students = asyncio.run(mongo_store.get_students_by_session_and_section(session_selector, selected_section))
            ...
            with st.form("edit_student_form"):
                ...
                with st.expander("Edit Pre-Test Data", expanded=False):
                    ...
                    for key in written_exam_scores:
                        written_exam_scores[key] = st.number_input(f"Pre-Test {key.replace('_', ' ').title()}", value=written_exam_scores.get(key, 0))
                ...
                with st.expander("Edit Post-Test Data", expanded=False):
                    ...
                    for key in post_written_exam_scores:
                        post_written_exam_scores[key] = st.number_input(f"Post-Test {key.replace('_', ' ').title()}", value=post_written_exam_scores.get(key, 0))
